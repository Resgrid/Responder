...
import { useShiftsStore, type ShiftViewMode } from '@/stores/shifts/store';
...
export default function ShiftsScreen() {
  ...
  const renderTabButton = (mode: ShiftViewMode, title: string) => (
    <Button
      ...
      className={`flex-1 ${currentView === mode
  ...
  const renderTodayShifts = () => {
    ...
    if (filteredTodaysShifts.length === 0) {
      return (
        <ZeroState
          title={t('shifts.no_shifts_today')}
          description={t('shifts.no_shifts_today')}
          buttonText=""
          onButtonPress={() => { }}
          showButton={false}
        />
    ...
    return (
      <FlatList
        ...
        renderItem={({ item }) => (
          <ShiftDayCard
            shiftDay={item}
            onPress={() => selectShiftDay(item)}
          />
        )}
        contentContainerStyle={styles.listContent}
        refreshControl={
          <RefreshControl
            refreshing={isTodaysLoading}
  ...
  const renderAllShifts = () => {
    ...
    if (filteredShifts.length === 0) {
      return (
        <ZeroState
          title={t('shifts.no_shifts')}
          description={t('shifts.no_shifts')}
          buttonText=""
          onButtonPress={() => { }}
          showButton={false}
        />
    ...
    <View className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <VStack className="flex-1">
        ...
        <View className="flex-1">
          {currentView === 'today' ? renderTodayShifts() : renderAllShifts()}
        </View>
      </VStack>
      ...
      <ShiftDayDetailsSheet
        isOpen={isShiftDayDetailsOpen}
        onClose={closeShiftDayDetails}
      />
    </View>
  );
}
