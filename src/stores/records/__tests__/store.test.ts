...
describe('Field Records store conformance', () => {
  ...
  it('refuses to stage a protected definition on the device', () => {
    useRecordsStore.setState({ catalog: { ContractVersion: 'field-catalog.v1', OriginClient: 'Responder', Ok: true, Reasons: [], ContextVerified: true, Definitions: [goldenCatalogEntry({ RequiresProtectedGrant: true })], Exclusions: [], ServerTimestampMs: 0 } });
  ...
  it('keeps a conflicted draft and never replays it in the batch push', async () => {
    useRecordsStore.setState({ catalog: { ContractVersion: 'field-catalog.v1', OriginClient: 'Responder', Ok: true, Reasons: [], ContextVerified: true, Definitions: [goldenCatalogEntry()], Exclusions: [], ServerTimestampMs: 0 } });
