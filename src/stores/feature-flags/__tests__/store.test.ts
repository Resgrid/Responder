...
// Capture the reset callback registered at module load (var: assigned inside the hoisted
// mock factory during import, and immune to jest.clearAllMocks unlike jest.fn call records).
// eslint-disable-next-line no-var
var mockFeatureFlagsResetFn: (() => void) | undefined;
jest.mock('@/lib/storage/clear-all-data', () => ({
  registerStoreReset: (_name: string, fn: () => void) => {
    mockFeatureFlagsResetFn = fn;
  },
}));
...
describe('Feature Flags Store', () => {
  ...
  describe('logout reset', () => {
    it('should clear flags, resolution and identity so the next session fails closed', () => {
      featureFlagsStore.setState({
        flags: { [FeatureFlagKeys.ChatSystem]: { enabled: true, value: null } },
        isLoaded: true,
      ...
      expect(mockFeatureFlagsResetFn).toBeDefined();
      mockFeatureFlagsResetFn?.();
