...
describe('AudioService', () => {
  ...
  const clearMockCalls = () => {
    mockSound.seekTo.mockClear();
    mockSound.play.mockClear();
    mockSound.remove.mockClear();
  ...
  describe('playStartTransmittingSound', () => {
    ...
    it('should queue playback if the player is still loading', async () => {
      clearMockCalls();
      mockSound.isLoaded = false;

      await audioService.playStartTransmittingSound();

      expect(mockSound.seekTo).not.toHaveBeenCalled();
      expect(mockSound.play).toHaveBeenCalled();
      mockSound.isLoaded = true;
    });
