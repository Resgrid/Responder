...
export const UnitsFilterSheet: React.FC = () => {
  ...
  useEffect(() => {
    if (isFilterSheetOpen && groupedFilterOptions) {
      try {
        ...
        trackEvent('units_filter_sheet_viewed', {
        ...
        console.warn('Failed to track units filter sheet view analytics:', error);
      }
    }
  ...
  const handleToggleFilter = useCallback(
    (filterId: string) => {
      ...
      try {
        trackEvent('units_filter_toggled', {
          timestamp: new Date().toISOString(),
          filterId,
        ...
        console.warn('Failed to track units filter toggle analytics:', error);
      }
