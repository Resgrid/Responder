...
export function LoginInfoBottomSheet({ isOpen, onClose, onSubmit }: LoginInfoBottomSheetProps) {
  ...
  return (
    <Actionsheet isOpen={isOpen} onClose={handleClose}>
      <ActionsheetBackdrop />
      {/* Single sanctioned keyboard mechanism for sheets: pad the bottom-anchored,
          content-sized sheet by the keyboard height so it slides up out from under the
          keyboard. Never nest a KeyboardAvoidingView here — see use-keyboard-height.ts. */}
      <ActionsheetContent className="rounded-t-3xl bg-white px-4 dark:bg-neutral-900" style={{ paddingBottom: keyboardHeight }}>
        ...
        <VStack space="lg" className="mt-4 w-full">
          <FormControl isRequired isInvalid={!!errors.username}>
            <FormControlLabel>
              <FormControlLabelText className="text-sm font-medium text-neutral-700 dark:text-neutral-200">{t('settings.username')}</FormControlLabelText>
            </FormControlLabel>
            <Controller
              control={control}
              name="username"
          ...
          <FormControl isRequired isInvalid={!!errors.password}>
            <FormControlLabel>
              <FormControlLabelText className="text-sm font-medium text-neutral-700 dark:text-neutral-200">{t('settings.password')}</FormControlLabelText>
            </FormControlLabel>
            <Controller
              control={control}
              name="password"
