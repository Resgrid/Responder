...
import { getWeatherAlertCategoryName, isSevereWeatherAlert, normalizeWeatherAlertSeverity } from '@/components/weather-alerts/weather-alert-formatters';
...
export const WeatherAlertBanner: React.FC = () => {
  ...
  // Subscribe to `alerts` itself. The store's getSevereAlerts() reads get().alerts, so
  // calling it in render creates no subscription and the banner would go stale when an
  // alert arrives or clears.
  const alerts = useWeatherAlertsStore((state) => state.alerts);
  const severeAlerts = useMemo(() => alerts.filter((alert) => isSevereWeatherAlert(alert.Severity)), [alerts]);
