      ...
      <Card className="mb-3 shadow-sm">
        <CardContent className="p-4">
          <VStack space="sm">
            ...
            {/* Description preview */}
            {item.Description ? (
              <Text className="text-gray-500 dark:text-gray-400 text-sm" numberOfLines={2}>
                {item.Description}
              </Text>
            ) : null}
            ...
            {item.Attendees && item.Attendees.length > 0 ? (
              <HStack className="items-center" space="sm">
                ...
                <Text className="text-gray-600 dark:text-gray-300 text-sm">
                  {t('calendar.attendeesCount', { count: item.Attendees.length })}
                </Text>
              </HStack>
            ) : null}
            ...
            {/* Signup info */}
            {canSignUp ? (
              <HStack className="justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-700">
                <Text className="text-sm text-gray-500 dark:text-gray-400">
                  {t('calendar.signupAvailable')}
                </Text>
                {isSignedUp ? (
